...
- **[MUST] 파괴적 조치 시 Human-in-the-loop 필수화:**
  > You MUST forcibly insert a Human-in-the-loop (approval from a domain expert) step into the workflow using Interactive Buttons in Slack/Teams when automating destructive actions like deleting/restarting resources or modifying policies, rather than leaving it 100% autonomous.
...
- **[MUST] Secrets Management:** 환경 변수나 에이전트 로직 내에 API Key, Token 등을 하드코딩하는 것을 NEVER use emojis. 모든 시크릿은 AWS Secrets Manager 또는 HashiCorp Vault를 통해 동적으로 주입(Inject) 받도록 보안 아키텍처를 강제하십시오.
