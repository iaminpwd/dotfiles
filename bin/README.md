...
|---|---|---|
| **`pre-flight-check.sh`** | 스테이징/수정된 파일 종류에 맞춰 `shellcheck`, `tflint`, `checkov`, `ansible-lint`, `hadolint`, `trivy`, `infracost` 등을 통합 실행하는 고성능 검증 훅. 변경 파일 중 `log-edit.sh`로 근거가 아직 보강되지 않은 항목은 비차단 경고로 알림 | `bin/hooks/pre-flight-check.sh` |
| **`compact-runner.sh`** | AI 에이전트의 토큰 폭주를 막기 위해 통과한 검증 항목을 `-> [✓] <경로>` 한 줄로 접고 실패 시 원형 로그를 보존하는 검증 래퍼 | `bin/hooks/compact-runner.sh` |
...
| 스크립트 | 역할 및 핵심 기능 | 실행 예시 |
|---|---|---|
| **`log-edit.sh`** | 파일 변경 시 근거 룰북 및 수정 목적 1줄을 `.agent-state/edits.log`에 기록 | `bin/utils/log-edit.sh <file> <rule_source> <purpose>` |
| **`prompt-flywheel.sh`** | 자가 검증 연속 실패 시 프롬프트 개정안 역제안 | `bin/utils/prompt-flywheel.sh` |
