...
# 아주 단순화된 정적 분석 로직 (MVP 개선)
# 3306이나 5432를 여는 블록 내에 cidr_blocks = ["0.0.0.0/0"] 이 있는지 검사 (하위 디렉토리 포함)
MATCHES=$(find "$TARGET_DIR" -type f -name "*.tf" -print0 | xargs -0 awk 'BEGIN{RS=""; FS="\n"} /(3306|5432)/ && /0\.0\.0\.0/' 2>/dev/null || true)
if [ -n "$MATCHES" ]; then
