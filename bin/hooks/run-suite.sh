...
if [ "${#SCRIPTS[@]}" -eq 0 ]; then
  ...
  # 숨김 디렉토리(.shared, .git 등) 하위는 제외한다. 호출부인 Justfile의 `just test`는
  # 셸 글롭(contexts/*/tests/run.sh)으로 대상을 모으는데 글롭은 dotglob 없이는 숨김
  # 디렉토리를 건너뛰는 반면 find는 그러지 않아, 같은 저장소에서 `just test`(12개)와
  # `just verify`/CI/pre-push(13개)가 서로 다른 집합을 돌리고 있었다. 그 차이가 당시의
  # 폐기 스킬 보관소(contexts/.archive/)라서, 쓰지도 않는 스킬의 회귀 테스트가
