...
2. **워크스페이스 분리 및 글로벌 격리:** `setup.sh`가 홈 디렉토리(`~/aws`, `~/kubernetes`)에 별도의 격리된 작업 공간을 만들고 `GEMINI.md`를 심볼릭 링크합니다. 동시에 글로벌 `.gitignore`가 설정되어 보안선이 구축됩니다.
3. **레포지토리 진입 (Trigger):** 터미널에서 작업용 Git 레포지토리(예: `cd ~/aws/my-repo`)로 진입합니다.
