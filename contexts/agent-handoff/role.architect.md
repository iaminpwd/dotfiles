...
- **[MUST] Blueprint Format**: 기본적으로 아래의 4가지 핵심 섹션을 마크다운으로 구성하십시오.
  ...
  3. `## 3. Action Plan`: 실행 에이전트가 즉시 적용할 수 있는 구체적인 단계별 지시.
     코드 스니펫은 **오로지 이 섹션 안에만** 배치하십시오.
     **반영 여부를 기계가 수렴 판정할 수 있는 작업**(예: "이 테스트가 통과할
     때까지 고쳐라")은 스니펫 전량 기재 대신 목표·제약·판정 명령만 적으십시오.
