...
## 1. 핵심 설계 원칙
- **[MUST] End-to-End Context Propagation:** 서비스 간 호출(HTTP, gRPC, 메시지 큐) 전 구간에서 W3C `traceparent` 헤더 전파가 완벽히 유지되도록 설계할 것.

## 2. 세부 오퍼레이션 조항 (Actionable Rules)

### 2.1 계측 및 샘플링
- **[MUST] Auto-Instrumentation First:** 언어별 OpenTelemetry Auto-Instrumentation Agent를 반드시 선제 적용하고, 커스텀 Span은 비즈니스적으로 의미 있는 경계(결제 처리, 외부 API 호출 등)에만 필요한 곳에만 수동 추가할 것.
- **[MUST] Tail-Based Sampling for Errors:** Head-based 샘플링만 적용할 경우 에러 트레이스가 누락될 수 있으므로, Collector 단에서 에러/고지연 트레이스를 100% 보존하는 Tail-Based Sampling 정책을 구성할 것.
