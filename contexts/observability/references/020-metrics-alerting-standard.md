...
- **[MUST] Burn Rate Alerting:** Error Budget 소진 속도(Burn Rate)를 단기(1h)/장기(6h) 윈도우로 동시 관측하여 오탐과 지연 탐지를 모두 예방할 것.
...
- **[PREFER] Suppress Known Noise:** 정기 배치 작업이나 배포 시점의 예측 가능한 스파이크는 `for:` 지속 시간 조정 또는 억제(Silence) 규칙을 적용하여 알람 오탐을 원천적으로 예방할 것.
