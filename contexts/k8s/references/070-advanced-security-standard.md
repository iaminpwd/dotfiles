...
## 1. 핵심 설계 원칙
- **[MUST] Threat Detection:** 실행 중인(Runtime) 컨테이너의 탈옥(Escape)이나 의심스러운 시스템 콜을 실시간 방어하도록 eBPF 기반의 Cilium Tetragon 또는 Falco를 DaemonSet으로 배포할 것.
- **[MUST] Read-Only Root Filesystem:** 악성 셸 스크립트 다운로드 및 디스크 파일 무결성을 보장하기 위해, 컨테이너 `securityContext` 내에 `readOnlyRootFilesystem: true` 지정을 의무화할 것. (쓰기가 필요한 임시 경로는 `emptyDir` 마운트로 격리 우회할 것)
- **[MUST] Image Signature Verification:** Kyverno/OPA Gatekeeper 등의 Admission Controller에서 Cosign 서명이 검증된 이미지에 한해서만 배포를 승인하도록 설계할 것. 이미지 서명 생성 자체는 `containers` 스킬(공급망 보안 표준)로 위임할 것.

## 2. 세부 오퍼레이션 조항 (Actionable Rules)

### 2.1 런타임 보안 방어 및 최소 권한
- **[MUST] Dropping Capabilities:** 최소 권한 원칙(PoLP)에 따라 컨테이너 기본 Capabilities(`ALL`)를 비활성화(`drop`)하고, 필수적인 특수 권한(`NET_BIND_SERVICE` 등)만 `add` 블록에 명시적으로 최소 기재할 것.

### 2.2 소프트웨어 공급망 보안
- **[MUST] Vulnerability Admission Control:** 배포 직전 이미지에 심각도 CRITICAL 수준의 CVE 취약점이 포함되어 있을 경우 Kubernetes API 서버 단에서 객체 생성 자체를 거부(Deny)하도록 Trivy Operator나 OPA Gatekeeper를 기반으로 동적 통제 정책을 구성할 것.
