...
- **[MUST] 중단 조건 (Halt Conditions):**
  - 클러스터 내부에 DB나 MQ 같은 상태 저장 미들웨어를 배포하면서, 전용 Operator CRD 없이 원시 StatefulSet으로 작성된 매니페스트가 감지될 시 즉시 작업을 중단(Halt & Clarify)하고 Operator 적용을 통보하십시오.
  - Multi-tenant 네임스페이스 설계 시 다른 테넌트의 리소스 명세를 변조할 수 있는 와일드카드 RBAC (`Role` 내 `resources: ["*"]`, `verbs: ["*"]`) 권한이 감지되면 즉시 작업을 멈추고 보안 룰을 세분화하십시오.
