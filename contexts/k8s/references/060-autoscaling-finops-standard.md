...
- **[MUST] 검증 도구 매핑:** `.tf` 변경이 없어 `infracost` 사전 게이트가 트리거되지 않는 K8s 매니페스트(HPA/VPA 등) 수정 시에는, Kubecost/OpenCost 실측치로 사후 비용 검증을 대체하십시오. 코드 사전 검증은 `contexts/pre-flight-check/SKILL.md`가 지정한 단일 래퍼 명령으로 일괄 수행하십시오.
