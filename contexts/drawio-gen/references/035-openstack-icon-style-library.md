...
OpenStack이 실제로 발행한 다이어그램 작성 표준(색상/선/도형 규칙)을 정의함.
...
- **[MUST] 강조색은 밝은 원색 3종 한정**: `light blue`(#4A90D9) / `red`(#DA1A32) /
  `green`(#2E7D32)만 사용할 것. 파스텔/보라/주황 등 원색이 아닌 색이나, 카테고리마다
...
- **[MUST]** 닫힌 기하 도형 또는 아이콘만 사용하고, 라벨 텍스트가 들어갈 여백을
  반드시 확보할 것. (아이콘 자체는 표준상 허용되나, 1절의 이유로 OpenStack은
...
- **[MUST]** `layout_toolkit.py`의 `edge()`는 `dashed=True`/`dotted=True` 불리언
  ...
  지금까지처럼 "논리적 연결"이라는 이유로 전부 `dashed=True`로 명확히 분리할 것.
...
- **[MUST] 정확한 매핑 적용**: 매핑 테이블에 없는 서비스는 억지로
  끼워 맞추는 대신 2절의 기능형 블록으로 대체하거나 사용자에게 확인할 것.
...
- **[MUST] 강조는 3색 중 하나만, 테두리로만**: 정말 강조가 필요할 때만
  `emphasis="blue"|"red"|"green"`을 지정한다. 배경까지 원색으로 채우는 대신
...
- **[MUST] 동일 기능 = 동일 강조색 재사용**: 예) "API 진입점"이라는 동일 역할의
  ...
  같은 강조색(예: red)을 재사용하고, 서비스마다 새 색상을 생성하는 대신 기존 색상을 재사용할 것.
- **[MUST] `{SHAPE}` 도형 토큰 — `rounded` / `cylinder` 2종만 사용**:
  ...
  제한을 받지 않는다.
  - **[MUST] 사전 정의된 도형 사용**: 이전 판본은 `hexagon`을
...
- **[MUST]** OpenStack 네이티브 서비스가 아닌 오픈소스/서드파티 도구(Jenkins/ArgoCD/
  ...
  `040-third-party-icon-library.md`를 그대로 적용할 것. (OSS 아이콘 매핑의 단일 진실 공급원은 040으로 통일되어 있으므로 본
  절에서 중복 서술을 생략함).
...
- **[MUST] 컨테이너도 0절 색상 기준을 적용할 것 (예외 조항 폐기)**: 이전 판본은
  "컨테이너는 객체가 아니라 구조적 그룹핑 경계이므로 0절 적용 대상이 아니다"라는
  예외를 두었으나, **공식 원문에는 그런 예외의 근거가 없습니다.** 원문은 객체를
  *"a closed geometric shape ... must have a hollow middle, where text can be added"*로
  정의하는데 swimlane 컨테이너는 이 정의에 정확히 부합함. 따라서 컨테이너 테두리도
  **검정(기본) 또는 밝은 원색 3종(`#4A90D9`/`#DA1A32`/`#2E7D32`)** 안에서 고르고,
  회색(`#888888`/`#555555`) 같은 무채색을 사용하는 대신 지정된 색상을 적용할 것.
  ...
  - drawio 기본 도형 색(예: 클라우드 스텐실의 `fillColor=#dae8fc`/`strokeColor=#6c8ebf`)을
    그대로 두는 대신 흰 채움 + 검정 테두리로 교체하여 명확히 할 것.
...
- **[MUST] Glance 는 VM 이 아니라 Nova 에 이미지를 제공**
  ...
  화살표를 그리는 대신 `Nova ↔ Glance` 왕복(조회/제공)으로 그리십시오. 공식 개념도도
  `Glance --provides images--> Nova` 형태임.
