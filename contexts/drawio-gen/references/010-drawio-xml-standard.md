...
- **[MUST] 공용 툴킷 재사용**: 아래 원칙을 매번 새로 구현하지 말고 `~/dotfiles/contexts/drawio-gen/scripts/layout_toolkit.py`를 `import`해서 쓰십시오. `grid()`/`hstack()`/`vstack()`/`offset_by_header()`/`subnet_box_size()`가 이미 구현되어 있고, `validate(path)`가 090 검증(ID 중복/끊어진 참조) + 형제 노드 겹침 검사를 한 번에 수행합니다.
...
- **[PREFER] 다이어그램 종횡비 가드레일**: 전체 다이어그램이 세로로 지나치게 길어지지 않도록 관리하십시오(세로:가로 비율 1.4:1 이하 권장). 한 컨테이너 내부의 서브넷이 5개를 초과하면 2열 고정 그리드보다 콘텐츠 크기 티어(위 규칙)에 맞춘 다열 배치를 우선 검토하십시오.
...
- **[MUST] 폭이 넓은 타겟에는 `entryX`/`entryY`를 반드시 명시하고, 접점 좌표로 검산**: 버스 바나 대형 컨테이너처럼 폭이 큰 도형을 타겟으로 삼으면 접점을 지정하지 않은 엣지가 전부 그 도형의 중앙 한 점으로 모여 부챗살이 됩니다. 소스의 `exitX`와 타겟의 `entryX`를 **같은 절대 x가 되도록** 계산해 넣으면 수직 스텁이 보장됩니다. 그리고 `render_preview()`는 exit/entry 스타일을 무시하고 항상 타겟 박스의 중심으로 선을 그리므로 **미리보기로는 이 문제를 검증할 수 없습니다** — `exitX*소스폭+소스x` 와 `entryX*타겟폭+타겟x` 를 직접 계산해 일치하는지 스크립트로 확인하십시오(2026-07-25, openstack-basic에서 실측 확인).
- **[MUST] 버스 바(가로 막대) 표기는 소비자가 한 줄로 늘어설 수 있을 때만**: 공유 인프라를 가로 막대로 그리고 소비자들이 수직 스텁으로 붙는 표기는 깔끔해 보이지만, **소비자 그룹이 세로로 쌓인 레이아웃에서는 성립하지 않습니다.** 바 2개(예: DB·MQ)를 쓰면 아래쪽 소비자가 위쪽 바에 닿기 위해 다른 바를 가로질러야 하고, 이를 피하려 그룹을 재배치하면 **배치 순서가 도메인 의미가 아니라 배선 편의로 정해지는 주객전도**가 발생합니다. 실사례(2026-07-25, openstack-basic): 버스 표기로 전환했다가 Placement가 맨 왼쪽, Glance가 Nova와 Neutron 사이에 끼는 무의미한 순서가 나와 전량 폐기했다. 세로 스택 레이아웃에서는 "대표 엣지 1개 + 공통 라벨"(§7)을 쓰십시오.
