...
- **[MUST] 버스 바(가로 막대) 표기는 소비자가 한 줄로 늘어설 수 있을 때만**: 공유 인프라를 가로 막대로 그리고 소비자들이 수직 스텁으로 붙는 표기는 깔끔해 보이지만, **소비자 그룹이 세로로 쌓인 레이아웃에서는 성립하지 않습니다.** 바 2개(예: DB·MQ)를 쓰면 아래쪽 소비자가 위쪽 바에 닿기 위해 다른 바를 가로질러야 하고, 이를 피하려 그룹을 재배치하면 **배치 순서가 도메인 의미가 아니라 배선 편의로 정해지는 주객전도**가 발생합니다. 실사례(2026-07-25, openstack-basic): 버스 표기로 전환했다가 Placement가 맨 왼쪽, Glance가 Nova와 Neutron 사이에 끼는 무의미한 순서가 나와 전량 폐기했다. 세로 스택 레이아웃에서는 "대표 엣지 1개 + 공통 라벨"(010 §7)을 쓰십시오.
