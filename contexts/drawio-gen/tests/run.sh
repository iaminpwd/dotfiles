...
# 선언이 없어 신규 환경에는 없다. 예전 진입점은 그 ImportError 가 SystemExit 앞에서
# 터져, 검증을 통과한 파일과 위반한 파일이 똑같이 exit 1 로 끝났다(2026-07-28 실측).
# 090 §2~3 이 이 CLI 의 종료 코드를 완료 조건의 기계 판정으로 쓰므로 회귀를 고정한다.
...
# ---------------------------------------------------------------------------
# check_icon_urls 회귀: 죽은 링크와 네트워크 불가를 구분하는지
# ---------------------------------------------------------------------------
# urlopen 이 4xx 를 HTTPError 로 올리는데 예전에는 그것을 일반 except 가 삼켜, 죽은
# 링크가 전부 [INFO](확인 불가)로 강등됐다. 문서(090 §7)가 약속한 [WARN] 이 한 번도
