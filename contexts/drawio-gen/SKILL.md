...
- **[MUST] 좌표는 공용 툴킷으로 계산**: 서브넷/컨테이너/아이콘 좌표를 손으로 하나씩 대입하지 말고 `~/dotfiles/contexts/drawio-gen/scripts/layout_toolkit.py`를 import해서 `grid`/`hstack`/`vstack`/`offset_by_header`/`subnet_box_size`로 계산하십시오(010 §10). 헤더 높이 오프셋 누락, 형제 컨테이너 높이 불일치, 콘텐츠 대비 과도한 여백은 전부 이 툴킷을 안 쓰고 좌표를 즉흥적으로 정할 때 발생한 실제 재발 버그입니다.
