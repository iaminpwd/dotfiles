...
- **[MUST] 검증 도구 매핑:** `shellcheck <script>` 및 `[ -L <symlink> ] && echo "OK" || echo "BROKEN"`을 통해 스크립트와 심볼릭 링크의 유효성을 검사하십시오. 이 도구들의 통합 실행 절차는 `contexts/pre-flight-check/SKILL.md`가 SSOT이므로, 개별 호출에 앞서 그 문서가 지정한 단일 명령으로 일괄 수행하십시오.
...
- **[MUST] 중단 조건 (Halt Conditions):**
  - 근본 원인 추적(`bash -x`, `echo $PATH`) 없이 `.zshrc` 하단에 경로를 무조건 추가하려는 임시방편적(Ad-hoc) 패턴이 감지되면 즉시 작업을 중단(Halt & Clarify)하고 선제적 추적을 먼저 수행하십시오.
  - 파일 내용 유실 위험이 있는 `rm -f` 또는 `>` 덮어쓰기 명령을 백업이나 `diff` 사전 비교 없이 바로 실행하려는 패턴이 감지되면 즉시 멈추고 백업 절차를 주입하십시오.
