...
#
# git/.githooks/pre-commit은 bin/*.sh 와 달리 test-coverage-check.sh 게이트 밖에 있었다.
# 그 사각지대에서 실제로 깨져 있던 것이 BIN_REMINDERS 재현 명령 조립 로직이다: 사람이
# 읽을 설명 문구 "run.sh (전체 — ...)"를 그대로 커맨드 문자열에 이어붙여, 출력된 줄을
# 그대로 복붙하면 괄호 때문에 셸 문법 오류가 나는 상태로 방치돼 있었다(2026-08-05 실측
# 발견 · 수정). 이 스위트는 그 재현 명령이 실제로 유효한 셸 명령인지와, 스테이징된
# 파일이 디스크에서 사라진 경우를 막는 보안 가드를 고정한다.
#
