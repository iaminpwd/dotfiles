...
- **[PREFER] Policy-as-Code:** Keystone `policy.yaml`(oslo.policy) 커스터마이징 시 기본 정책보다 완화되지 않게 방어 로직을 적용하여 검토하고, 변경은 코드로 버전 관리하십시오.
...
<example>
[Good]
- "DB 보안 그룹의 3306 포트 인그레스를 애플리케이션 보안 그룹(remote group `sg-app`)으로만 제한하십시오."
- "SSH 22번 포트 인그레스 소스를 사내 VPN 대역(`10.10.0.0/16`)으로만 한정하십시오."
</example>
<example>
[Bad]
