...
- **[MUST] 완료 조건 (Done when):** `pre-flight-check.sh` 스캔을 통해 탐지된 네트워크 취약점이나 노출 포트가 무결하게 해결되어야 합니다. 포트 추가/삭제, 보안 그룹 신규 생성 등 중대한 변경이 발생할 경우에만 완화 내역을 포함한 `security-audit-report.md`를 작성할 것.
