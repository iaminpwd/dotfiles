...
### 2.1 아키텍처 설계 기획 표준
- **[MUST] Agentic RAG:** 설계 전 에이전트 스스로 `grep_search`나 `view_file`을 사용하여 `030`(FinOps), `060`(Magnum) 등 사내 표준 프롬프트 룰을 능동 조사하여 반영하십시오.
- **[MUST] Project Foraging:** 설계 착수 전 반드시 `run_command`로 `openstack token issue`, `openstack quota show`, `openstack network list` 등을 실행하여 대상 프로젝트의 실제 상태와 잔여 쿼터를 팩트 기반으로 확보하십시오.
