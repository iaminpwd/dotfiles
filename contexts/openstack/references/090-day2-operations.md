...
- **[MUST] 검증 도구 매핑:** GitHub Actions는 `actionlint`, Ansible 배포는 `ansible-lint`로 구문을 검증하고, `openstack --version` 및 서비스 상태(`openstack compute service list`)로 배포 결과를 확인하십시오. 이 도구들의 통합 실행 절차는 `contexts/pre-flight-check/SKILL.md`가 SSOT이므로, 개별 호출에 앞서 그 문서가 지정한 단일 명령으로 일괄 수행하십시오.
