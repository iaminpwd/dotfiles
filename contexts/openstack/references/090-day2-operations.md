...
- **[MUST] 검증 도구 매핑:** GitHub Actions는 `actionlint`, Ansible 배포는 `ansible-lint`로 구문을 검증하고, `openstack --version` 및 서비스 상태(`openstack compute service list`)로 배포 결과를 확인하십시오.
