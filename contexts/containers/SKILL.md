...
이 스킬은 Dockerfile 작성, 컨테이너 이미지 빌드/하드닝, 공급망 보안, 레지스트리 관리 작업 시 발동됩니다. K8s 오케스트레이션 자체(매니페스트, HPA 등)는 `k8s` 스킬을 참조하십시오.
...
- **[MUST] 사전 룰북 및 연관 참조 연쇄 분석 (Recursive Reference Check)**: Dockerfile이나 이미지 빌드 파이프라인 코드를 작성하기 전, 반드시 라우팅 테이블에서 대상 룰북을 찾아 먼저 읽으십시오. 또한 해당 룰북 내에 명시된 연관 참조 문서(`references:` 항목 또는 텍스트 내 참조 문서)가 존재하는 경우 연쇄적으로 읽되, 이미 읽은 파일은 중복 방문하지 않는 방문 목록(Visited Set) 규칙을 준수하여 무한 루프 없이 연결된 모든 연관 룰북을 빠짐없이 수집하십시오.
