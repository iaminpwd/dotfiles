...
- **[MUST] 검증 도구 매핑:** 빌드된 이미지에 대한 취약점 스캔은 `bash ~/dotfiles/contexts/containers/scripts/container-hardening-gate.sh <Image>` 를 실행하여 자동 판정하십시오. 그 외 구성 파일 검증은 `contexts/pre-flight-check/SKILL.md`가 지정한 단일 래퍼 명령으로 일괄 수행하십시오.
