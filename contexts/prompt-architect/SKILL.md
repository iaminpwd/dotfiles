...
- **[MUST] 린터 자가 검증 (Linter Regression Test)**: `contexts/scripts/prompt-lint.sh` 로직 자체를 수정한 직후에는 반드시 `bash ~/dotfiles/contexts/prompt-architect/tests/run.sh`를 실행해 회귀 테스트를 통과해야 한다.
