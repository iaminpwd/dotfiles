...
#### 3.3.1. 시스템 지시어 권한 부여 (System Instructions)
**이론:** 시스템 지시어(System Instructions)를 활용해 가장 강력한 베이스 룰을 모델 뇌리에 깊숙이 주입하는 아키텍처.

**통합 워크스페이스 적용 사례:**
```markdown
- 전체 룰북을 동적으로 병합한 뒤 `<system_instructions>` 마스터 태그 안에 가두어 Gemini에 전달하는 setup.sh 스크립트 아키텍처.
```

#### 3.3.2. Zero-Config 태그 내재화 (Encapsulation)
**이론:** 외부 빌드 스크립트가 파일명이나 하드코딩된 조건으로 태그를 주입하는 대신, 각 지식 모듈(마크다운 파일) 스스로가 자신의 특성 태그(예: `<domain_specific_rules>`)를 내장하도록 책임을 위임하는 객체지향적 프롬프트 관리 기법.

**통합 워크스페이스 적용 사례:**
```markdown
- `setup.sh`는 파일명 숫자에 의존하지 않고, 오직 `grep`을 통해 모듈 내부에 태그가 포함되어 있는지 동적으로 감지하여 글로벌 룰과 도메인 룰을 스마트하게 분리 병합함.
```
...
**이론:** 수많은 도메인 룰(010~100)들이 동시에 로드되었을 때, 지시 간 모순이 발생할 경우 에이전트가 우왕좌왕하지 않고 스스로 계급을 판단하여 하위 룰을 가차 없이 덮어쓰도록(Override) 강제하는 논리 회로.

**통합 워크스페이스 적용 사례:**
- 각 파일 최상단 XML 태그의 `priority` 속성(`highest`, `critical`, `high`)을 기계적으로 해석.
- 000번 마스터 코어(`highest`)의 룰은 그 어떤 예외도 허용하지 않는 **절대 타협 불가능한 헌법(Hard Constraint)**으로 작동하여 모든 충돌을 종식시킵니다.
...
**통합 워크스페이스 적용 사례:**
- 논리적 엣지 케이스 발견 시, 스스로 `.contexts/*.md` 파일의 허점을 진단하고 사용자에게 "프롬프트 마크다운 원본의 리팩토링"을 역제안(Reverse Proposal)합니다.
