...
- **[MUST] CI/CD Local Test:** GitHub Actions 등 파이프라인 코드 작성 시 반드시 `run_command`로 `act -W <특정_워크플로우_파일>` 도구를 실행하여 로컬에서 사전 검증하십시오.
...
- **[MUST] Data Resilience:** 데이터베이스의 고가용성(Zone-Redundant) 및 백업/복원 전략은 `080-database-standard` 모듈의 규칙을 준수하십시오. 추가로, 악의적 삭제나 휴먼 에러에 대비한 연속 백업 활성화를 기본값으로 설정하십시오.
...
- **[MUST] Actionable Alerts:** 모든 알람에는 즉시 실행 가능한 런북(Runbook) 링크를 제공하거나 Action Group, Event Grid, Azure Functions를 연동한 자동화된 조치(Automated Remediation) 파이프라인을 반드시 함께 제안하십시오.
