...
- **[PREFER] TGW:** 글로벌 확장성 확보를 위해 Azure Transit Gateway(TGW) 기반의 중앙 집중형 라우팅을 적극 제안하십시오.
- **[MUST] State Management:** State 저장은 반드시 Azure Blob Storage Backend와 Cosmos DB State Locking을 사용하여 원격으로 안전하게 구성하십시오.
...
terraform {
  backend "azurerm" {
    ...
    storage_account_name = "terraform-locks"
  }
}
```
</example>
...
- **[MUST] SG Lazy Deletion Control:** Azure Functions 등 VNet ENI와 강하게 결합되는 Security Group을 다룰 때는, Azure의 네트워크 인터페이스(NIC) 지연 삭제 과정 등에서 안정적인 리소스 수명 주기 제어(Lifecycle Management)를 보장하기 위해 `name_prefix = "..."`를 적극 사용하고, `lifecycle { create_before_destroy = true }` 블록을 필수로 포함하십시오.
