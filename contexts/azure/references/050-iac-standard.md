...
- **[MUST] Static Code Analysis (tflint 검증):** Terraform 코드 검증 시, 로컬에 `tflint` 도구가 설치되어 있다면 `run_command`로 `tflint --init` 및 `tflint` 명령어를 실행하여 Azure Provider 문법 및 Best Practice 위반 사항이 없는지 정적 분석을 수행하고, 미설치 시에는 사용자에게 경고 후 진행하십시오.
