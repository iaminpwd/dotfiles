...
## 1. 핵심 설계 원칙
- **[MUST] Decoupling:** Terraform은 인프라 리소스 수명 주기 관리, Ansible은 OS 설정 및 앱 구성 담당으로 역할을 엄격히 분리할 것.
...
- **[MUST] Dynamic Mapping:** 가용 영역(AZ)은 `data "azurerm_availability_zones"` 블록 등을 활용하여 동적으로 매핑할 것.
...
- **[MUST] Resource Iteration:** 다수 리소스 반복 생성 시 `for_each`를 활용하되, `known after apply`인 동적 값을 반복할 때는 반드시 정적 식별자(Static Key)를 갖는 `map` 구조를 강제하여 Plan 실행의 안정성을 보장할 것.
...
- **[MUST] NSG Lazy Deletion Control:** Azure Functions 등 VNet NIC와 결합되는 Network Security Group(NSG)을 다룰 때는 NIC 지연 삭제 과정에서 안정적인 리소스 제어를 위해 `name_prefix`를 사용하고 `lifecycle { create_before_destroy = true }` 블록을 필수로 포함할 것.

### 2.2 Ansible 엔지니어링 표준
- **[MUST] Idempotency:** 패키지 설치 시 `state: present`를 명시하고, `yum`, `systemd`, `file` 등 전용 모듈을 최우선으로 사용하여 멱등성을 달성할 것.
- **[MUST] Dynamic Inventory:** 인벤토리 구성 시 반드시 Azure VM Dynamic Inventory Plugin(`azure_rm.yml`)을 활용할 것.
- **[MUST] Vault:** 민감한 변수(DB 패스워드 등)는 Ansible Vault로 암호화할 것.

### 2.3 엔터프라이즈 명명 규칙 및 태깅
- **[MUST] Naming & FinOps Tagging:** 모든 리소스 이름은 `<Project>-<Env>-<Service>-<Resource>` 규칙을 따르고, `Project`, `Environment`, `CostCenter` 필수 태그를 지정하여 자동으로 하위 리소스에 상속되도록 설정할 것.
...
- **[MUST] 검증기 수정 시 회귀 테스트 선통과:** `pre-flight-check.sh`의 Terraform 검증 로직을 고칠 때는 `bash ~/dotfiles/contexts/azure/tests/run.sh`를 먼저 실행해 전부 통과하는지 확인할 것. 각 픽스처는 조항 하나씩을 재현합니다(예: `fail-open-ssh`는 4절 SSH 개방 중단 조건, `fail-unpinned-version`은 2.1절 Version Pinning). 새 검증 로직을 추가할 때는 위반을 재현하는 픽스처와 기대 결과를 `tests/`에 함께 등록할 것.

## 4. 도메인 특화 자가 비판 및 중단 조건 (Self-Critique & Halt Conditions)
- **[Trigger: Before Terraform Apply] 사전 조치 및 점검 기준 (절차는 010-azure-core.md의 공통 자가 비판 절차 참조):** 상태 변경 명령어를 실행하기 전 반드시 `terraform plan -input=false`를 먼저 실행할 것.
...
- **[MUST] 중단 조건 (Halt Conditions):**
  ...
  - SSH(22포트) 등 민감 포트가 `0.0.0.0/0`으로 과도하게 개방되는 보안 규칙이 감지되면 즉시 작업을 멈추고 보안 위반을 보고할 것.
  - NIC 지연 삭제 오류 등으로 인해 리소스 삭제 명령어 실행이 3회 연속 실패할 경우 즉시 작업을 중단하고 수동 분리를 요청할 것.
