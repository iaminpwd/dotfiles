...
### 2.1 트러블슈팅 및 장애 진단
- **[PREFER] Deep Dive Analysis:** 단순 표면 오류 문구 수집을 넘어, 필요시 Application Insights, NSG Flow Logs 추적 데이터를 다각도로 수집하여 입출력 양방향 흐름을 교차 검증할 것.
...
- **[PREFER] Incident Coordination:** 다중 팀 간 대응 조율이 필요한 대형 장애는 Azure Monitor Action Groups와 ITSM 커넥터를 통해 온콜 호출과 대응 조치 자동화를 표준화할 것.
...
- **[MUST] 검증 도구 매핑:** `az monitor log-analytics query` 및 KQL 쿼리를 활용하여 팩트 로그 상태를 기계적으로 추출할 것.
...
- **[MUST] 중단 조건 (Halt Conditions):**
  - 문제 분석 및 데이터 수집 시, 로컬에 로그 수집 등 핵심 CLI 도구(`az cli`)가 누락되어 있거나 인증 권한 오류(`AccessDenied`)로 인해 팩트 수집이 3회 연속 실패할 경우 즉시 작업을 중단(Halt & Clarify)하고 권한을 요청할 것.
