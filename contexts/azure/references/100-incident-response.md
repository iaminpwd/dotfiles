...
본 모듈은 서비스 장애 인시던트 발생 시의 긴급 복구, 근본 원인 분석(RCA) 및 비난 없는 사후 분석(Blameless Post-Mortem) 보고서 작성 시 적용되는 표준 가이드라인입니다.

## 1. 핵심 설계 원칙
- **[MUST] Mitigation First:** 장애 상황 인지 즉시 서비스 정상화(복구/롤백) 조치를 최우선으로 실행하십시오. 원인 파악 및 코드 수정은 긴급 복구가 완료된 뒤 수행하십시오.
- **[MUST] Active Data Gathering:** 반드시 로컬 `az cli` 또는 Azure Monitor Log Analytics 조회 결과만을 팩트로 삼아 원인을 파악하십시오.
- **[MUST] Blameless RCA:** 휴먼 에러를 방어하지 못한 시스템적 결함(예: `prevent_destroy` 누락, RBAC 관리자 권한 남용 등)을 역추적하여 개선책을 마련하십시오.

## 2. 세부 오퍼레이션 조항 (Actionable Rules)

### 2.1 트러블슈팅 및 장애 진단
- **[PREFER] Deep Dive Analysis:** 단순 표면 오류 문구 수집을 넘어, 필요시 Application Insights, NSG Flow Logs 추적 데이터를 다각도로 수집하여 입출력 양방향 흐름을 교차 검증하십시오.
...
- **[PREFER] Incident Coordination:** 다중 팀 간 대응 조율이 필요한 대형 장애는 Azure Monitor Action Groups와 ITSM 커넥터를 통해 온콜 호출과 대응 조치 자동화를 표준화하십시오.
...
- **[MUST] 완료 조건 (Done when):** 수집된 에러 로그 데이터가 `<grounding_check>` 검증을 거쳐 `troubleshooting-report.md` 및 `post-mortem-report.md`에 결함 없이 정리되고, 재발 방지를 위한 구체적 액션 아이템이 파일 링크 형태로 보증되어야 합니다.
...
- **[MUST] 중단 조건 (Halt Conditions):**
  - 문제 분석 및 데이터 수집 시, 로컬에 로그 수집 등 핵심 CLI 도구(`az cli`)가 누락되어 있거나 인증 권한 오류(`AccessDenied`)로 인해 팩트 수집이 3회 연속 실패할 경우 즉시 작업을 중단(Halt & Clarify)하고 권한을 요청하십시오.
