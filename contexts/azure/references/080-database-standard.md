...
- **[MUST] 검증 도구 매핑:** `tflint` 또는 `checkov`를 활용해 DB 관련 IaC 파일의 암호화 미설정 및 백업 정책 누락을 자동 스캔하십시오. DB 인바운드 보안 그룹 규칙 소스가 특정 Web/WAS 서브넷 및 애플리케이션 보안 그룹(ASG)으로만 제한되어 있는지 확인하십시오. 이 도구들의 통합 실행 절차는 `contexts/pre-flight-check/SKILL.md`가 SSOT이므로, 개별 호출에 앞서 그 문서가 지정한 단일 명령으로 일괄 수행하십시오.
