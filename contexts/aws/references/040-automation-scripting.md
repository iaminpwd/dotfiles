...
- **[MUST] 검증 도구 매핑:** `bash -n <script.sh>`를 통해 기본 구문 오류를 검증하고, `shellcheck`를 실행하여 쉘 스크립트 정적 분석 및 위험 요소를 스캔하십시오. 이 도구들의 통합 실행 절차는 `contexts/pre-flight-check/SKILL.md`가 SSOT이므로, 개별 호출에 앞서 그 문서가 지정한 단일 명령으로 일괄 수행하십시오.
