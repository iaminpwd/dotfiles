...
- **[PREFER] Information Foraging:** 리소스 ID(VPC, Subnet 등)는 반드시 터미널에서 `aws ec2` API 등으로 실제 인프라 상태를 선제적으로 조회하여 팩트 기반으로 확보하십시오. 실제 AWS API 조회 결과(팩트)를 동적으로 참조하여 기재하십시오.
