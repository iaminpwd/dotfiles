...
본 모듈은 AWS RDS, Aurora, NoSQL DynamoDB 및 인메모리 캐시 ElastiCache 설계와 구현 시 적용되는 기술 표준 가이드라인입니다.

## 1. 핵심 설계 원칙
- **[MUST] High Availability:** 프로덕션 RDS 및 Aurora 클러스터에는 반드시 Multi-AZ 배포 아키텍처를 적용하여 고가용성을 확보하십시오.
- **[MUST] Data Security:** 데이터베이스 스토리지 암호화(Encryption at Rest)를 활성화하고, 암호화 키는 AWS KMS 고객 관리형 키(CMK)를 지정하십시오.
...
- **[PREFER] Connection Management:** 접속자가 몰리는 고성능 웹 서비스 RDS 전면에는 커넥션 풀링 관리를 위해 RDS Proxy 배포를 설계하십시오.
...
- **[MUST] Data Lifecycle (TTL):** 세션 정보 등 임시 데이터 수집 테이블에는 비용 통제를 위해 DynamoDB TTL(Time To Live) 속성을 필수로 기재하십시오.

### 예시 코드 및 패턴 (Few-Shot Examples)
<examples>
<example>
[Good]
- "커넥션 병목을 줄이기 위해 RDS Proxy를 연동하여 자원을 최적화하십시오."
...
- **[MUST] 검증 도구 매핑:** `tflint` 또는 `checkov`를 활용해 DB 관련 IaC 파일의 암호화 미설정 및 백업 정책 누락을 자동 스캔하십시오. DB 인바운드 보안 그룹 규칙 소스가 특정 Web/WAS 보안 그룹 ID로만 제한되어 있는지 확인하십시오. 이 도구들의 통합 실행 절차는 `contexts/pre-flight-check/SKILL.md`가 SSOT이므로, 개별 호출에 앞서 그 문서가 지정한 단일 명령으로 일괄 수행하십시오.
...
- **[MUST] 중단 조건 (Halt Conditions):**
  - DB 리소스의 Public Access (`publicly_accessible = true`) 설정이 감지되거나 보안 그룹 상 DB 포트(3306, 5432 등)가 `0.0.0.0/0`에 노출되는 위험이 발견될 시 즉시 작업을 중단(Hard Block)하고 보안 경고를 발송하십시오.
  - KMS CMK 암호화 옵션(`storage_encrypted = false`)이 비활성화된 상태로 RDS 생성이 시도될 경우 작업을 즉시 멈추고 보안 수정을 강제하십시오.
