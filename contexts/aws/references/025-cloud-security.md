...
클라우드 네트워크 및 다중 계정 접근 통제 보안 표준임.

## 1. 핵심 설계 원칙
- **[MUST] Zero-Trust 인바운드 통제:** 웹 포트(80, 443) 외 모든 인바운드는 VPN 또는 특정 보안 그룹으로 격리할 것. (이유: 접근 통제)
...
- **[PREFER] WAF/Shield:** 퍼블릭 엔드포인트(ALB, CloudFront) 제안 시 AWS WAF와 Shield Advanced 적용을 포함할 것.
- **[MUST] Session Manager:** 인스턴스 직접 SSH 접근을 격리하고, AWS SSM Session Manager를 통하도록 설계를 제안할 것.
- **[MUST] VPC Endpoint:** AWS 내부 서비스 통신 시 퍼블릭 인터넷 경로 보안을 강화하기 위해 VPC Endpoint(Gateway/Interface)를 구성할 것.
- **[PREFER] IPAM:** 멀티 VPC/멀티 계정 환경에서 CIDR 네트워크 독립성을 보장하기 위해 Amazon VPC IP Address Manager(IPAM)를 통한 중앙 집중형 IP 주소 관리를 제안할 것.

### 2.2 엔터프라이즈 권한 통제
- **[MUST] Landing Zone Structure:** 신규 멀티 계정 환경 구축 시 AWS Control Tower(또는 Landing Zone Accelerator)를 기반으로 Management, Log Archive, Security Tooling, 워크로드(Prod/Non-Prod) 계정을 최소 단위로 분리한 계정 구조를 우선 설계할 것.
- **[PREFER] Federation (SSO):** 다중 계정 접근 통제를 위해 AWS IAM Identity Center (SSO) 기반의 중앙 집중형 연동 아키텍처를 최우선 제안할 것.
- **[PREFER] Threat Detection:** 엔터프라이즈 내부 네트워크 위협 탐지를 위해 Amazon GuardDuty 적용을 함께 제안할 것.
- **[PREFER] Continuous Compliance:** 리소스 설정 드리프트 상시 탐지를 위해 AWS Config Rules를, GuardDuty 등 여러 보안 서비스의 탐지 결과를 통합 관리하기 위해 AWS Security Hub를 함께 제안할 것.
