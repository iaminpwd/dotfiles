...
- **[MUST] 정본 글로벌 명령어 단일 실행:** 인프라 및 스크립트 코드가 수정된 후, 검증 대상 저장소 안에서 `run-suite.sh --pfc-args="--changed"` (또는 `just check`)를 실행하여 검증을 통합 수행할 것. `--pfc-args=` 는 `run-suite.sh` 전용 패스스루 플래그로, 뒤에 붙인 값을 내부적으로 `pre-flight-check.sh`에 그대로 전달합니다(`pre-flight-check.sh` 자체를 직접 실행할 때는 `--pfc-args=` 없이 `--changed`만 넘길 것). `--changed` 는 스테이징 여부와 무관하게 방금 수정한 변경분 전부(staged + unstaged + untracked)를 대상으로 삼습니다.
  ...
  *   이 `pre-flight-check.sh` 스크립트는 terraform fmt, init, validate를 완전히 대체(Replace)하는 통합 검증 파이프라인으로 작동합니다. 정적 분석(tflint), 보안/시크릿 스캔(trivy/trufflehog) 등이 함께 일괄 수행됩니다. **비용 분석(infracost breakdown 기반 Extended Support 연장 요금 검증)** 은 별도로 `RUN_COST_CHECK=true` 환경 변수가 설정된 경우에만 수행되며, 이 변수는 현재 `git/.githooks/pre-commit`(실제 git 커밋 시점)에서만 자동으로 설정됩니다. 위 명령을 터미널에서 수동으로 실행할 때는 비용 분석이 기본적으로 건너뛰어지므로, 커밋 훅 밖에서 비용 분석까지 확인하려면 `RUN_COST_CHECK=true run-suite.sh --pfc-args="--changed"`처럼 명시적으로 변수를 설정할 것.
