#!/usr/bin/env bash
# db-sg-checker.sh
# Terraform 코드를 정적 스캔하여 DB 인바운드 보안 그룹 소스가 Web/WAS로 제한되어 있는지 확인
...
if awk 'BEGIN{RS=""; FS="\n"} /(3306|5432)/ && /0\.0\.0\.0/' "$TARGET_DIR"/*.tf 2>/dev/null | grep -q .; then
  echo "[ERROR] DB 포트(3306/5432)가 0.0.0.0/0으로 열려있습니다. Web/WAS SG로 제한하십시오."
  FAILED=1
fi

# 이 스크립트에서는 개념 증명을 위해 특정 패턴을 차단하는 방식으로 구현했습니다.
