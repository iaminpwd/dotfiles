...
# k8s-check.sh, observability-check.sh 다섯 곳이 공유하는 QUIET 로깅 + 저장소 루트 판정
# SSOT다. 심볼릭 링크 배포 호환성은 test-tool-probe-ssot.sh 가 pre-flight-check.sh/
...
# 5. 소비자 5곳이 옛 인라인 로직(log_info 함수 재정의, REPO_ROOT cd 블록)을 되살리지
...
if [ "$dup" -eq 0 ]; then
  report "소비자 5곳 모두 공용 라이브러리를 source하고 인라인 복제 없음" 0
else
  report "소비자 5곳 모두 공용 라이브러리를 source하고 인라인 복제 없음" 1 "복제본이 되살아났거나 source가 빠졌습니다"
fi
