...
#
# 예전엔 이 파일 하나에 모든 섹션이 들어있었는데, git 훅에서 bin/ 변경을 감지해
# 회귀 테스트를 안내하려면 "무엇이 바뀌면 어느 서브스위트를 봐야 하는지"가 1:1로
# 추적 가능해야 한다. 한 파일에 뭉쳐 있으면 안내가 뭉뚱그려질 수밖에 없어서,
# bin/linters/*.sh 등 대상 스크립트 1개당 서브스위트 파일 1개로 쪼갰다
# (2026-08-04). 각 서브스위트는 독립적으로도 실행 가능하다.
#
...
FAILED=()
for suite in test-run-suite test-tool-probe-ssot test-script-init test-parallel-pair test-plugin-loop test-finops test-db-sg test-idempotency test-shell test-yaml test-provenance-reminder; do
